  ...
  class ReceiptTemplate {
    ...
    /// Build ESC/POS bytes untuk barcode label 33x15mm, 2 kolom per baris
    static Future<Uint8List> buildBarcodeLabel({
      required List<Map<String, dynamic>> items,
      ...
      // Proses 2 item per baris (kiri & kanan)
      for (int i = 0; i < items.length; i += 2) {
        // === LABEL KIRI ===
        bytes += _buildSingleLabel(items[i], dayCode, withPrice);

        // === LABEL KANAN (jika ada) ===
        if (i + 1 < items.length) {
          bytes += _buildSingleLabel(items[i + 1], dayCode, withPrice);
        }

        // Cut after each row
        bytes += [0x1D, 0x56, 0x42, 0x00]; // Partial cut
      }

      return Uint8List.fromList(bytes);
    }

    /// Build ESC/POS bytes untuk 1 label
    static List<int> _buildSingleLabel(
      ...
      final kode = item['kode']?.toString() ?? '0';
      final harga = (item['harga'] ?? 0).toDouble();

      // ===== NAMA ITEM =====
      result += [0x1B, 0x61, 0x01]; // Center align
      result += [0x1D, 0x21, 0x00]; // Font normal
      result += [0x1B, 0x4D, 0x00]; // Font A (small)
      result += _textToBytes(nama, maxLength: 20);
      result += [0x0A]; // New line

      // ===== BARCODE CODE128 =====
      result += [0x1B, 0x61, 0x01]; // Center align
      result += [0x1D, 0x68, 40];   // Barcode height: 40 dots
      result += [0x1D, 0x77, 2];    // Barcode width: 2
      result += [0x1D, 0x48, 0];    // HRI position: none
      result += [0x1D, 0x6B, 0x49]; // CODE128 type
      result.add(kode.length);       // Data length ← PERBAIKAN: .add() bukan +=
      result += kode.codeUnits;      // Barcode data
      result += [0x0A];              // New line

      // ===== INFO ROW: Kode + DayCode + Harga =====
      result += [0x1B, 0x61, 0x00]; // Left align
      result += [0x1D, 0x21, 0x00]; // Font normal

      String infoLine = kode;
      if (withPrice) {
        infoLine += ' Rp.${_formatCurrencyIntNoSymbol(harga.toInt())}';
      }
      infoLine += ' $dayCode';

      result += _textToBytes(infoLine, maxLength: 32);
      result += [0x0A]; // New line

      // Feed sedikit untuk jarak
      result += [0x1B, 0x4A, 2]; // Feed 2 dots

      return result;
    }

    /// Convert text to bytes dengan truncate
    static List<int> _textToBytes(String text, {int maxLength = 20}) {
      String display = text.length > maxLength ? text.substring(0, maxLength - 2) + '..' : text;
      return display.codeUnits;
    }

    /// Format currency tanpa simbol Rp
    static String _formatCurrencyIntNoSymbol(int amount) {
