...
class _DoFormScreenState extends State<DoFormScreen>
  ...
  bool get isPusat {
    ...
    return currentCabang?.kode == '00' ||
        currentCabang?.jenis.toLowerCase() == 'pusat';
  }
    ...
    _loadCabangTujuanList().then((_) {
      if (widget.mutasiHeader != null) {
        ...
        _keteranganController.text =
            widget.mutasiHeader!['mutc_keterangan'] ?? '';
        _loadMutasiDetail();
      }
    });
  ...
  void dispose() {
    ...
    for (var controller in _ketControllers.values) { controller.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _processBarcode(String barcode) async {
    ...
    // Tidak ditemukan — jika bukan pusat, cari dari API
    if (!isPusat) {
      ...
      try {
        ...
        if (itemData != null) {
          ...
          _showToast('Item ditambahkan: ${newItem.itemNama}',
              type: ToastType.success);
          HapticFeedback.mediumImpact();
        } else {
          _showToast('Item dengan ID $itemId tidak ditemukan',
      ...
      _showToast('Item tidak ada dalam daftar permintaan',
          type: ToastType.error);
      HapticFeedback.heavyImpact();
    }
  }
  ...
  Future<void> _loadGudangList() async {
    ...
    try {
      ...
      _showToast('Gagal memuat data gudang: ${e.toString()}',
          type: ToastType.error);
    } finally {
  ...
  Future<void> _loadCabangTujuanList() async {
    ...
    try {
      final cabangData =
      await DoService.getCabangTujuanList(currentCabang.kode);
      setState(() => _cabangTujuanList = cabangData);
    } catch (e) {
      _showToast('Gagal memuat data cabang tujuan: ${e.toString()}',
          type: ToastType.error);
    } finally {
  ...
  Future<void> _loadMintaList() async {
    ...
    try {
      final startDate =
      DateFormat('yyyy-MM-dd').format(DateTime(2024, 1, 1));
      ...
      setState(() => _mintaList = mintaData);
    } catch (e) {
      _showToast('Gagal memuat data permintaan: ${e.toString()}',
          type: ToastType.error);
    } finally {
  ...
  Future<void> _loadMintaDetail(String nomor) async {
    ...
    try {
      ...
      final details =
      List<Map<String, dynamic>>.from(mintaDetail['details']);
      ...
      setState(() {
        _items = details.map((detail) {
          ...
          if (rawQty is int) {
            ...
            qtyMinta = rawQty.toInt();
          } else if (rawQty is String) {
            qtyMinta = int.tryParse(rawQty) ?? 0;
          }
      ...
      _showToast('Berhasil load ${_items.length} item dari permintaan',
          type: ToastType.success);
    } catch (e) {
      _showToast('Gagal memuat detail permintaan: ${e.toString()}',
          type: ToastType.error);
    } finally {
      setState(() => _isLoadingMinta = false);
    }
    ...
    try {
      ...
      setState(() {
        ...
        if (targetCabangKode != null && _cabangTujuanList.isNotEmpty) {
          final matchingCabang = _cabangTujuanList
              .cast<Map<String, dynamic>>()
              .firstWhere(
                (c) => c['cbg_kode'] == targetCabangKode,
            orElse: () => {},
          );
          _selectedCabangTujuan =
          matchingCabang.isNotEmpty ? matchingCabang : null;
        } else {
      ...
      _showToast('Gagal memuat detail: ${e.toString()}',
          type: ToastType.error);
    } finally {
      setState(() => _isLoading = false);
    }
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredItems = _items
            .where((item) =>
            item.itemNama.toLowerCase().contains(searchLower))
            .toList();
      }
    });
  }
    ...
    setState(() {
      ...
      if (filteredIndex != -1) {
        _filteredItems[filteredIndex] =
            _filteredItems[filteredIndex].copyWith(qty: newQty);
      }
    });
  }
  ...
  void _updateItemKeterangan(int itemId, String keterangan) {
    setState(() {
      ...
      if (itemIndex != -1) {
        _items[itemIndex] =
            _items[itemIndex].copyWith(keterangan: keterangan);
      }
      final filteredIndex = _filteredItems
          .indexWhere((item) => item.itemId == itemId);
      if (filteredIndex != -1) {
        _filteredItems[filteredIndex] =
            _filteredItems[filteredIndex].copyWith(keterangan: keterangan);
      }
    });
  }
      ...
      builder: (context, child) => Theme(
        data: ThemeData.light().copyWith(
          colorScheme: ColorScheme.light(
              primary: _primaryDark, onPrimary: Colors.white),
  ...
  void _showSelectMintaDialog() {
    if (_selectedCabangTujuan == null) {
      _showToast('Pilih cabang tujuan terlebih dahulu!',
          type: ToastType.error);
      return;
    }
                  ...
                  child: _isLoadingMinta
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 12),
                        Text('Tidak ada data permintaan',
                            style: GoogleFonts.montserrat(
                                fontSize: 12,
    ...
    if (selectedItems != null && selectedItems.isNotEmpty) {
      setState(() {
        for (var newItem in selectedItems) {
          ...
          item.itemId == newItem.itemId && item.tipe == newItem.tipe);
          if (existingIndex >= 0) {
            final newQty = _items[existingIndex].qty + newItem.qty;
            _items[existingIndex] =
                _items[existingIndex].copyWith(qty: newQty);
            if (_qtyControllers.containsKey(newItem.itemId)) {
              _qtyControllers[newItem.itemId]?.text =
                  newQty.toString();
            }
          } else {
            _items.add(newItem);
            _qtyControllers[newItem.itemId] = TextEditingController(
                text: newItem.qty.toString());
            _ketControllers[newItem.itemId] = TextEditingController();
          }
        }
        _filteredItems = List.from(_items);
      });
      _showToast('${selectedItems.length} item ditambahkan',
          type: ToastType.success);
    }
  }
  ...
  void _showToast(String message, {required ToastType type}) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      backgroundColor:
      type == ToastType.success ? _accentMint : _accentCoral,
  ...
  Future<void> _saveMutasi() async {
    ...
    if (itemsWithQty.isEmpty) {
      _showToast('Minimal satu item harus memiliki quantity!',
          type: ToastType.error);
      return;
    }
    ...
    try {
      ...
      if (isPusat && _selectedMinta != null) {
        ...
        requestData['cabang_database'] =
        _selectedCabangTujuan!['cbg_database'];
      }
  ...
  int get _totalItemsWithQty =>
      _items.where((item) => item.qty > 0).length;
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            if (_nomorMutasi != null) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border:
                  Border.all(color: _primaryDark.withOpacity(0.2)),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(Icons.confirmation_number_rounded,
                        size: 12, color: _primaryDark),
            ...
            Row(
              children: [
                Expanded(child: _buildGudangDropdown()),
          ...
          Expanded(
            child: _isLoadingGudang
              ...
              child: DropdownButton<Gudang>(
                ...
                onChanged: (gudang) {
                  setState(() => _selectedGudangAsal = gudang);
                },
                icon: Icon(Icons.arrow_drop_down,
                    color: _accentSky, size: 20),
                style: GoogleFonts.montserrat(
                    fontSize: 12, color: _textDark),
              ),
            ),
          ...
          Expanded(
            child: _isLoadingCabang
              ...
              child: DropdownButton<Map<String, dynamic>>(
                ...
                isExpanded: true,
                hint: Text('Cabang Tujuan',
                    style: GoogleFonts.montserrat(
                        fontSize: 12, color: _textLight)),
                items: _cabangTujuanList
                ...
                onChanged: (cabang) {
                  ...
                  if (isPusat) _loadMintaList();
                },
                icon: Icon(Icons.arrow_drop_down,
                    color: _accentGold, size: 20),
                style: GoogleFonts.montserrat(
                    fontSize: 12, color: _textDark),
              ),
            ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                    _selectedMinta != null
                        ? _selectedMinta!['mt_nomor']
                        : 'Belum dipilih',
                    style: GoogleFonts.montserrat(
                        fontSize: 12,
                        fontWeight: FontWeight.w600,
                        color: _selectedMinta != null
                            ? _accentGold
                            : _textLight)),
              ],
            ),
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(
                        flex: 3, child: _buildSearchField()),
                    const SizedBox(width: 8),
                    Expanded(
                        flex: 2, child: _buildBarcodeField()),
                  ],
                ),
                const SizedBox(height: 10),
                Row(
                  children: [
                    ...
                    const Spacer(),
                    _buildStatChip(Icons.inventory_rounded,
                        '${_items.length}'),
                    const SizedBox(width: 4),
                    _buildStatChip(Icons.shopping_cart_rounded,
                        '$_totalQuantity',
                        color: _accentGold),
                    const SizedBox(width: 4),
                    _buildStatChip(Icons.check_circle_rounded,
                        '$_totalItemsWithQty',
                        color: _accentMint),
                  ],
                ),
                ...
                if (_filteredItems.isEmpty)
                  ...
                  ListView.separated(
                    ...
                    itemBuilder: (context, index) =>
                        _buildModernItemCard(_filteredItems[index]),
                  ),
              ],
            ),
          ),
  ...
  Widget _buildSearchField() {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: _borderSoft)),
      child: Center( // ✅ Tambah Center wrapper
        child: TextField(
          controller: _searchController,
          textAlignVertical: TextAlignVertical.center, // ✅ Center vertikal
          style: GoogleFonts.montserrat(fontSize: 11),
          onChanged: _filterItems,
          decoration: InputDecoration(
            hintText: 'Cari item...',
            hintStyle:
            GoogleFonts.montserrat(fontSize: 11, color: _textLight),
        ...
        child: Column(
          children: [
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(spacing: 4, runSpacing: 4, children: [
                        ...
                        _badge(item.tipeLabel,
                            color: item.tipeColor),
                        if (hasMinta)
                          _badge('Minta: ${item.qtyMinta}',
                              color: _accentSky),
                        if (hasQty)
                          _badge('Kirim: ${item.qty}',
                              color: _accentGold),
                ...
                SizedBox(
                  ...
                  child: TextField(
                    ...
                    onChanged: (value) => _updateItemQty(
                        item.itemId, int.tryParse(value) ?? 0),
                  ),
                ),
              ],
            ),
            ...
            TextField(
              ...
              style:
              GoogleFonts.montserrat(fontSize: 10, color: _textDark),
              decoration: InputDecoration(
                hintText: 'Keterangan...',
                hintStyle: GoogleFonts.montserrat(
  ...
  Widget _badge(String text, {Color? color}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(4),
        border: Border.all(
            color: (color ?? _primaryDark).withOpacity(0.3)),
      ),
      child: Text(text,
          style: GoogleFonts.montserrat(
              fontSize: 8,
        ...
        children: [
          ...
          Text(
              isPusat && _selectedMinta == null
                  ...
                  : (_searchController.text.isEmpty
                  ? 'Tambah item dengan Add Item atau scan barcode'
          ...
          children: [
            Expanded(
              child: Column(
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text('Item dengan QTY',
                      style: GoogleFonts.montserrat(
                          fontSize: 9, color: _textLight)),
                  Row(
                    children: [
                      Icon(Icons.check_circle_rounded,
                          size: 12, color: _accentMint),
                      const SizedBox(width: 4),
                      Text(
...
class _AddItemModalMutasiState extends State<AddItemModalMutasi> {
  ...
  String _getControllerKey(int id, String tipe) => '${id}_$tipe';
  ...
  Color _getTipeColor(String tipe) =>
      tipe == 'BJ' ? _accentSky : _accentMint;
  String _getTipeLabel(String tipe) =>
      tipe == 'BJ' ? 'Barang Jadi' : 'Setengah Jadi';
  IconData _getTipeIcon(String tipe) => tipe == 'BJ'
      ? Icons.inventory_2_outlined
      : Icons.precision_manufacturing_outlined;
          ...
          Expanded(
            child: _isLoading
              ...
              itemBuilder: (context, index) {
                ...
                final isExisting = widget.existingItems.any(
                        (i) => i.itemId == itemId && i.tipe == tipe);
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(8)),
                        child: Icon(_getTipeIcon(tipe),
                            size: 18, color: tipeColor),
                      ),
                      const SizedBox(width: 10),
                      Expanded(
                        child: Column(
                            ...
                            if (isExisting)
                              Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    borderRadius: BorderRadius.circular(4)),
                                child: Text('Sudah ada di daftar',
                                    style: GoogleFonts.montserrat(
                                        fontSize: 8, color: _accentGold)),
                              ),
                          ],
                        ),
                      ),
