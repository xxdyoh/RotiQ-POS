...
class _SpkListScreenState extends State<SpkListScreen> {
  ...
  Future<void> _loadSpkData() async {
    ...
    try {
      ...
      _showSnackbar('Gagal memuat data SPK: ${e.toString()}', Colors.red);
    } finally {
  ...
  void _showEditKeteranganDialog(Map<String, dynamic> spk) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            const Icon(Icons.edit, color: Color(0xFFF6A918), size: 20),
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            Text('Hapus SPK?', style: GoogleFonts.montserrat(fontSize: 14)),
          ],
        ),
        content: Text(
          'Apakah Anda yakin ingin menghapus "${spk['spk_nomor']}"?',
          style: GoogleFonts.montserrat(fontSize: 12),
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: Colors.red, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 6)),
  ...
  void _showDetailBottomSheet(Map<String, dynamic> spk) {
  ...
  Widget _buildDetailBottomSheet(Map<String, dynamic> header, List<Map<String, dynamic>> details) {
    final isClosed = header['spk_isclosed'] == 1;

    final totalQty = details.fold<int>(0, (sum, item) {
      final qty = item['spkd_qty'];
    ...
    final totalRealisasi = details.fold<int>(0, (sum, item) {
      final realisasi = item['qty_realisasi'];
      if (realisasi is int) return sum + realisasi;
      if (realisasi is double) return sum + realisasi.toInt();
      if (realisasi is String) return sum + (int.tryParse(realisasi) ?? 0);
      return sum;
    });
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Icon(
                        isClosed ? Icons.check_circle : Icons.assignment,
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Text(
                          DateFormat('dd MMM yyyy').format(DateTime.parse(header['spk_tanggal'])),
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        Row(
                          children: [
                            Expanded(child: _buildInfoItem('Keterangan', header['spk_keterangan'] ?? '-')),
                            Expanded(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: DataTable(
                          ...
                          columns: const [
                            ...
                            DataColumn(label: Text('Qty SPK', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                            DataColumn(label: Text('Realisasi', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                            DataColumn(label: Text('Sisa', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                          ],
                          rows: details.asMap().entries.map((entry) {
                            final index = entry.key + 1;
                            final item = entry.value;

                            final qty = item['spkd_qty'] ?? 0;
                            final realisasi = item['qty_realisasi'] ?? 0;

                            int qtyInt = 0;
                            if (qty is int) qtyInt = qty;
                            else if (qty is double) qtyInt = qty.toInt();
                            else if (qty is String) qtyInt = int.tryParse(qty) ?? 0;

                            int realisasiInt = 0;
                            if (realisasi is int) realisasiInt = realisasi;
                            else if (realisasi is double) realisasiInt = realisasi.toInt();
                            else if (realisasi is String) realisasiInt = int.tryParse(realisasi) ?? 0;

                            final sisa = qtyInt - realisasiInt;
                            ...
                            return DataRow(
                              cells: [
                                ...
                                DataCell(Text(qtyInt.toString(), style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600))),
                                DataCell(Text(realisasiInt.toString(), style: const TextStyle(fontSize: 10, color: Colors.green))),
                                DataCell(Text(
                                  sisa.toString(),
                                  style: TextStyle(
                                    fontSize: 10,
                                    color: sisa > 0 ? Colors.orange : Colors.grey,
                                  ),
                                )),
                              ],
                            );
                    ...
                    child: SfDataGrid(
                      ...
                      onCellTap: (details) {
                        if (details.rowColumnIndex.rowIndex > 0) {
                          ...
                          if (dataRowIndex >= 0 && dataRowIndex < _spkList.length) {
                            final data = _spkList[dataRowIndex];
                      ...
                      columns: [
                        ...
                        GridColumn(
                          ...
                          label: Container(
                            padding: const EdgeInsets.only(left: 4, top: 4),
                            alignment: Alignment.centerLeft,
                            child: const Text('User', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11)),
                          ),
                        ),
                        GridColumn(
                          columnName: 'aksi',
  ...
  SpkDataSource({
    ...
    _data = spkList.asMap().entries.map((entry) {
      ...
      return DataGridRow(cells: [
        ...
        DataGridCell<String>(columnName: 'nomor', value: data['spk_nomor']?.toString() ?? '-'),
        DataGridCell<String>(columnName: 'tanggal', value: _formatDate(data['spk_tanggal'])),
        DataGridCell<String>(columnName: 'keterangan', value: data['spk_keterangan']?.toString() ?? '-'),
        DataGridCell<String>(columnName: 'status', value: data['spk_isclosed'] == 1 ? 'Closed' : 'Open'),
        DataGridCell<String>(columnName: 'user_create', value: data['user_create']?.toString() ?? '-'),
