...
class _StokinListScreenState extends State<StokinListScreen> with SingleTickerProviderStateMixin {
  final GlobalKey<SfDataGridState> _key = GlobalKey<SfDataGridState>();
  final DataGridController _dataGridController = DataGridController();

  // Warna modern dari StokinFormScreen
  ...
  Future<void> _loadStokinData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _calculateTotals(stokinData);
        _dataSource = StokinDataSource(
          stokinList: stokinData,
      ...
      _showToast('Gagal memuat data stock in: ${e.toString()}', type: ToastType.error);
    } finally {
      setState(() => _isLoading = false);
    }
  }

  void _calculateTotals(List<Map<String, dynamic>> stokinList) {
    _totalFilteredStokin = stokinList.length;
  }
  ...
  void _onFilterChanged(DataGridFilterChangeDetails details) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_dataSource.effectiveRows != null) {
        ...
        setState(() {
          _totalFilteredStokin = filteredData.length;
        });
      }
    });
  }
  ...
  void _showItemDetailDialog(Map<String, dynamic> stokin) {
    showDialog(
      context: context,
        ...
        child: Container(
          ...
          child: Column(
            children: [
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.receipt, size: 18, color: Colors.white),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  DateFormat('dd MMMM yyyy').format(DateTime.parse(stokin['sti_tanggal'])),
                                  style: GoogleFonts.montserrat(
                                    fontSize: 11,
                                    fontWeight: FontWeight.w600,
                                    color: _textDark,
              ...
              Expanded(
                child: FutureBuilder(
                  future: StokinService.getStokInDetail(stokin['sti_nomor'].toString()),
  ...
  Future<void> _exportToExcel() async {
    try {
      ...
      sheet.name = 'Data Stock In';

      // Set column widths
      sheet.getRangeByIndex(1, 1).columnWidth = 6;   // No
      sheet.getRangeByIndex(1, 2).columnWidth = 18;  // Nomor
      sheet.getRangeByIndex(1, 3).columnWidth = 10;  // Tanggal
      sheet.getRangeByIndex(1, 4).columnWidth = 35;  // Keterangan
      ...
      sheet.getRangeByName('B$totalRow').setText('$_totalFilteredStokin Transaksi');
      ...
      if (kIsWeb) {
        ...
        final anchor = html.AnchorElement(href: url)
          ..target = 'blank'
          ..download = 'StockIn_List_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.xlsx'
          ..click();
        ...
        final file = File('${directory.path}/StockIn_List_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.xlsx');
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                        width: 90, // Sedikit lebih lebar untuk menampung icon + teks
                        height: 40,
                        decoration: BoxDecoration(
                          gradient: LinearGradient(
                            colors: [_accentMint, _accentMint.withOpacity(0.8)],
                          ...
                          child: ClipRRect(
                            ...
                            child: SfDataGrid(
                              ...
                              columns: [
                                ...
                                GridColumn(
                                  columnName: 'tanggal',
                                  width: _columnWidths['tanggal'] ?? 140,
                                  minimumWidth: 140,
                                  maximumWidth: 900,
                                  label: Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 4),
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        const SizedBox(width: 4),
                                        Text(
                                          '$_totalFilteredStokin Transaksi',
                                          style: GoogleFonts.montserrat(
                                            fontSize: 10,
                                            fontWeight: FontWeight.w600,
                                            color: _primaryDark,
                                  ...
                                  Container(
                                    ...
                                    padding: const EdgeInsets.symmetric(horizontal: 4),
                                    alignment: Alignment.center,
                                    child: _totalFilteredStokin < _stokinList.length
                                        ? Container(
                                      ...
                                      child: Text(
                                        '${_stokinList.length - _totalFilteredStokin} filter',
                                        style: GoogleFonts.montserrat(
                                          fontSize: 8,
                                          fontWeight: FontWeight.w600,
                                          color: _accentGold,
...
class StokinDataSource extends DataGridSource {
  StokinDataSource({
    ...
    _originalStokinList = stokinList;
    _updateDataSource(stokinList);
  }

  List<Map<String, dynamic>> _originalStokinList = [];
  ...
  void _updateDataSource(List<Map<String, dynamic>> stokinList) {
...
// DataSource untuk detail items di dialog
class StokinDetailDataSource extends DataGridSource {
  StokinDetailDataSource({
